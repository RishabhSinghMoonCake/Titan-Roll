...
public class Destructible : MonoBehaviour
{
    ...
    [Tooltip("The player Mass Level required to smash and destroy this object.")]
    public int targetMassLevel = 1;

    [Tooltip("The maximum speed penalty (0.0 to 1.0) applied when the player hits this object.")]
    [Range(0f, 1f)]
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("Boulder"))  
        {
            ...
            // --- 1. THE PERCENTAGE & THRESHOLD CHECK ---
            if (playerLevel < targetMassLevel)
            {
                // UNDER-LEVELED: Apply the full penalty
                damagePercentage = maxDamagePenalty;  


                // Apply the impact slowdown to the boulder 
                boulder.ApplyImpactSlowdown(damagePercentage);  

                // OPTIONAL ARCADE JUICE: Play a small dust spark or audio thud here since we hit a wall!
                if (dustEffectPrefab != null)
                {
                    ObjectPooler.Instance.Spawn(dustEffectPrefab, other.ClosestPoint(transform.position), Quaternion.identity);  
                }

                // <-- THE CRITICAL FIX: Stop right here! Do not smash, fracture, or destroy the object!
                return;
            }
            else
            {
                // SUCCESS: Player meets or exceeds the mass level!
